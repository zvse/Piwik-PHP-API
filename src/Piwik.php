...
class Piwik {
   ...
   * Set date range
   *
   * @param string $rangeStart e.g. 2012-02-10 (YYYY-mm-dd) or last5(lastX),
   *   previous12(previousY)...
   * @param string $rangeEnd e.g. 2012-02-12. Leave this parameter empty to
   *   request all data from
   ...
   *
   * @param bool $isJsonDecodeAssoc false decode as Object, true for decode as
   *   Associate array
   */
   ...
   * via the API method
   * `CustomDimensions.getAvailableScopes()`. This method will also contain
   * information whether actually Custom Dimension slots are available or
   * whether they are all already in use.
   *
   * @param string $name The name of the dimension
   * @param string $scope Either 'visit' or 'action'. To get an up to date list
   *   of available scopes fetch the API method
   *   `CustomDimensions.getAvailableScopes`
   * @param int $active '0' if dimension should be inactive, '1' if dimension
   *   should be active
   * @param array $optional
   *
   * @return bool|object
   ...
   * `CustomDimensions.configureNewCustomDimension`. The response also contains
   * information whether more Custom Dimensions can be created or not. Requires
   * at least Admin access for the specified website.
   *
   * @return bool|object
   */
   ...
   * @param bool|string $aliasedGraph "by default, Graphs are "smooth"
   *   (anti-aliased). If you are generating hundreds of graphs and are
   *   concerned with performance, you can set aliasedGraph=0. This will
   *   disable anti aliasing and graphs will be generated faster, but look less
   *   pretty."
   * @param string $idGoal
   * @param array $colors Use own colors instead of the default. The colors has
   *   to be in hexadecimal value without '#'
   * @param array $optional
   *
   * @return bool|object
   */
   ...
   * @param string $searchKeywordParameters Comma separated list of search
   *   keyword parameter names
   * @param string $searchCategoryParameters Comma separated list of search
   *   category parameter names
   * @param string $excludeIps Comma separated list of IPs to exclude from the
   *   reports (allows wildcards)
   ...
   * @param string $startDate Date at which the statistics for this website
   *   will start. Defaults to today's date in YYYY-MM-DD format
   * @param string $excludedUserAgents
   * @param string $keepURLFragments If 1, URL fragments will be kept when
   *   tracking. If 2, they will be removed. If 0, the default global behavior
   *   will be used.
   * @param string $settingValues JSON serialized settings eg {settingName:
   *   settingValue, ...}
   * @param string $type The website type, defaults to "website" if not set.
   * @param string $excludeUnknownUrls Track only URL matching one of website
   *   URLs
   * @param array $optional
   *
   * @return bool|object
   ...
   *
   * @param string $segment
   * @param string $hideFutureHoursWhenToday Hide the future hours when the
   *   report is created for today
   * @param array $optional
   *
   * @return bool|object
   */
